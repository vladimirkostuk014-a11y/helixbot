...
const UserCRM: React.FC<UserCRMProps> = ({ users, setUsers, config, commands = [], topicNames = {}, addLog }) => {
    ...
    const [customMuteTime, setCustomMuteTime] = useState('');
            ...
            if (delta > 0) {
                if (newWarns < 3) {
                    await apiCall('sendMessage', {
                        ...
                        text: `⚠️ @${selectedUser.username || selectedUser.name}, предупреждение (${newWarns}/3).`
                    }, config);
                }
                if (addLog) addLog('Варн', `Выдано предупреждение ${selectedUser.name} (${newWarns}/3)`, 'warning');
            } else {
                await apiCall('sendMessage', {
                    chat_id: config.targetChatId,
                    text: `🕊 @${selectedUser.username || selectedUser.name}, предупреждение снято. Счет: ${newWarns}/3.`
                }, config);
                if (addLog) addLog('Варн', `Снято предупреждение ${selectedUser.name} (${newWarns}/3)`, 'info');
            }
        } catch (e) {
    ...
    const handleSendPrivate = async () => {
        ...
        try {
            ...
            const newMsg = { 
                dir: 'out', 
                text: msgText, 
                type: (mediaFile || mediaUrl) ? 'photo' : 'text', 
        ...
        <div className="flex h-full bg-[#0c0c0e] rounded-2xl overflow-hidden border border-gray-800 shadow-2xl">
            ...
            <div className={`w-80 flex flex-col border-r border-gray-800 bg-[#121214] ${selectedUser ? 'hidden md:flex' : 'flex w-full'}`}>
                <div className="p-4 border-b border-gray-800 flex gap-2">
                    ...
                    <button onClick={exportToCsv} className="bg-gray-800 hover:bg-gray-700 p-2 rounded-xl text-blue-400 border border-gray-700" title="Экспорт в CSV">
                        <Icons.Upload size={18} className="rotate-180"/>
                    </button>
                </div>
                    ...
                    {getFilteredUsers().map((u: User) => (
                        <div key={u.id} onClick={() => setSelectedUserId(u.id)} className={`p-4 border-b border-gray-800/50 cursor-pointer hover:bg-gray-800/50 transition-all ${selectedUserId === u.id ? 'bg-blue-900/10 border-l-4 border-l-blue-500' : 'border-l-4 border-l-transparent'}`}>
                            <div className="flex items-center gap-3">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shrink-0 shadow-lg ${u.role === 'admin' ? 'bg-yellow-500 text-black' : 'bg-gray-700 text-gray-300'}`}>
                                    {u.name.charAt(0)}
                                </div>
                                ...
                                {u.unreadCount ? (
                                    <span className="bg-blue-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">{u.unreadCount}</span>
                                ) : u.warnings > 0 && (
                                    <span className="bg-red-900/50 text-red-400 text-[10px] font-bold px-2 py-0.5 rounded flex items-center gap-1">{u.warnings} <Icons.AlertTriangle size={8}/></span>
                                )}
                            </div>
                        </div>
                    ))}
            ...
            {selectedUser ? (
                <div className="flex-1 flex flex-col relative bg-[#09090b]">
                    ...
                    <div className="h-16 border-b border-gray-800 flex items-center justify-between px-6 bg-[#121214]">
                        <div className="flex items-center gap-4">
                            ...
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg ${selectedUser.role === 'admin' ? 'bg-yellow-500 text-black' : 'bg-gray-700 text-white'}`}>
                            ...
                            <div>
                                ...
                                <div className="text-xs text-gray-400 flex items-center gap-2">
                                    @{selectedUser.username || 'no_user'} 
                                    <span className={`px-1.5 rounded text-[10px] uppercase font-bold ${selectedUser.role === 'admin' ? 'bg-yellow-900/40 text-yellow-500' : 'bg-gray-800 text-gray-500'}`}>{selectedUser.role}</span>
                                </div>
                            </div>
                        </div>
                        <button onClick={handleClearHistory} className="text-gray-500 hover:text-red-500 p-2 rounded-lg transition-colors border border-gray-700/50 hover:bg-gray-800" title="Очистить историю с пользователем">
                    ...
                    <div className="flex flex-1 overflow-hidden">
                        ...
                        <div className="flex-1 flex flex-col min-w-0 bg-black/20 relative">
                                            ...
                                            <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm shadow-md transition-all ${isOut ? 'bg-blue-600 text-white rounded-br-sm' : 'bg-gray-800 text-gray-200 rounded-bl-sm border border-gray-700'}`}>
                                                ...
                                                {msg.buttons && msg.buttons.length > 0 && (
                                                    <div className="mt-2 pt-2 border-t border-white/20 flex flex-wrap gap-2">
                                                        {msg.buttons.map((b, bi) => <span key={bi} className="bg-black/20 px-2 py-1 rounded text-xs">{b.text}</span>)}
                                                    </div>
                                                )}
                                                <div className="text-[10px] opacity-50 text-right mt-1 flex justify-end items-center gap-1">
                                                    {msg.time}
                                                    {isOut && <Icons.Check size={10}/>}
                                                </div>
                                            </div>
                                        </div>
                                    );
                                })}
                                {(selectedUser.history || []).filter(m => !m.isGroup).length === 0 && (
                                    <div className="flex items-center justify-center h-full text-gray-600 text-sm">История сообщений пуста</div>
                                )}
                            </div>
                            ...
                            <div className="p-4 bg-[#121214] border-t border-gray-800 relative z-20">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={() => setShowTools(!showTools)} className={`p-3 rounded-xl transition-colors ${showTools ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'}`}>
                                        <Icons.Plus size={20}/>
                                    </button>
                                ...
                                <>
                                    ...
                                    <div className="mb-6 p-4 bg-black/30 rounded-xl border border-gray-800">
                                        <div className="flex justify-between items-center mb-2">
                                            <span className="text-sm font-bold text-white flex items-center gap-2"><Icons.AlertTriangle size={14} className="text-yellow-500"/> Варны</span>
                                            <span className="text-lg font-mono text-yellow-500">{selectedUser.warnings || 0}/3</span>
                                        </div>
                                        <div className="flex gap-2">
                                            <button onClick={() => handleWarn(-1)} className="flex-1 bg-gray-800 hover:bg-gray-700 text-white py-2 rounded-lg font-bold text-lg">-</button>
                                    ...
                                    <div className="mb-6 p-4 bg-black/30 rounded-xl border border-gray-800">
                                        <div className="flex justify-between items-center mb-3">
                                            <span className="text-sm font-bold text-white flex items-center gap-2"><Icons.Mic size={14} className="text-blue-400"/> Мут</span>
                                            {selectedUser.status === 'muted' && <span className="text-[10px] bg-yellow-900/50 text-yellow-500 px-2 rounded">Активен</span>}
                                        </div>
                                        <div className="grid grid-cols-2 gap-2 mb-2">
                                            <button onClick={() => handleMute(10)} className="bg-gray-800 hover:bg-gray-700 text-xs py-2 rounded text-gray-300">10 мин</button>
                                            <button onClick={() => handleMute(60)} className="bg-gray-800 hover:bg-gray-700 text-xs py-2 rounded text-gray-300">1 час</button>
                                    ...
                                    <div className="mb-6">
                                        <button 
                                            ...
                                            className={`w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${selectedUser.status === 'banned' ? 'bg-green-600 hover:bg-green-500 text-white shadow-lg shadow-green-900/20' : 'bg-red-600 hover:bg-red-500 text-white shadow-lg shadow-red-900/20'}`}
                                        >
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-gray-600 bg-[#0f0f12]">
                    <div className="w-24 h-24 bg-gray-900 rounded-full flex items-center justify-center mb-4 shadow-inner">
