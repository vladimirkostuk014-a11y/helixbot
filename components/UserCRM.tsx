...
const UserCRM: React.FC<UserCRMProps> = ({ users, setUsers, config, commands = [], topicNames = {}, addLog }) => {
    ...
    const getFilteredUsers = () => {
        return (Object.values(users) as User[]).filter((u: User) => {
            ...
            // Removed 'left' status check because left users are now deleted from DB
    ...
    const handleKickUser = async () => {
        ...
        if (window.confirm(`Вы точно хотите кикнуть ${selectedUser.name} из группы?`)) {
            try {
                ...
                // User will be removed by bot-server on 'left_chat_member' event
    ...
    // Helper to add button state logic
    ...
    const handleMute = async (minutes: number) => {
        ...
        await apiCall('restrictChatMember', { chat_id: config.targetChatId, user_id: selectedUserId, permissions: JSON.stringify({ can_send_messages: false }), until_date: until }, config);
    ...
    const handleUnmute = async () => {
         if (!selectedUserId) return;
         await apiCall('restrictChatMember', { chat_id: config.targetChatId, user_id: selectedUserId, permissions: JSON.stringify({ can_send_messages: true, can_send_media_messages: true, can_send_other_messages: true }) }, config);
         setUsers(prev => ({ ...prev, [selectedUserId]: { ...prev[selectedUserId], status: 'active' } }));
         saveData(`users/${selectedUserId}/status`, 'active');
    };
        ...
        <div className="flex h-full bg-[#0c0c0e] rounded-2xl overflow-hidden border border-gray-800 shadow-2xl">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="flex justify-between items-center">
                                        <div className="font-bold text-gray-200 truncate text-sm flex items-center gap-1">{u.name}</div>
            ...
            {selectedUser ? (
                        ...
                        <div className="flex-1 flex flex-col min-w-0 bg-black/20 relative h-full">
                            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 custom-scrollbar flex flex-col-reverse relative z-10">
                                ...
                                {[...(selectedUser.history || []).filter(m => !m.isGroup)].reverse().map((msg, i) => (
                                    <div key={i} className={`flex w-full ${msg.dir === 'out' ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-[85%] md:max-w-[75%] px-4 py-2.5 rounded-2xl text-sm shadow-md transition-all ${msg.dir === 'out' ? 'bg-blue-600 text-white rounded-br-sm' : 'bg-gray-800 text-gray-200 rounded-bl-sm border border-gray-700'}`}>
                            ...
                            <div className="p-4 bg-[#121214] border-t border-gray-800 relative z-20">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button onClick={handleSendPrivate} disabled={isSending} className="bg-blue-600 hover:bg-blue-500 text-white w-12 rounded-xl flex items-center justify-center transition-colors"><Icons.Send size={20}/></button>
                                </div>
                            </div>
                        </div>
                        <div className="w-full md:w-72 bg-[#121214] border-t md:border-t-0 md:border-l border-gray-800 p-6 overflow-y-auto custom-scrollbar">
                            {selectedUser.role !== 'admin' && (
                                <>
                                    ...
                                    <div className="mb-6"><button onClick={handleBanToggle} className={`w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${selectedUser.status === 'banned' ? 'bg-green-600 hover:bg-green-500 text-white' : 'bg-red-600 hover:bg-red-500 text-white'}`}>{selectedUser.status === 'banned' ? <><Icons.Check size={18}/> Разбанить</> : <><Icons.Slash size={18}/> ЗАБАНИТЬ</>}</button></div>
                                </>
                            )}
                             <div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-gray-600 bg-[#0f0f12] hidden md:flex">
                    <div className="w-24 h-24 bg-gray-900 rounded-full flex items-center justify-center mb-4"><Icons.Users size={48} className="opacity-20"/></div>
