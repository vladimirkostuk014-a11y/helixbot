...
const UserCRM: React.FC<UserCRMProps> = ({ users, setUsers, config, commands = [], topicNames = {}, addLog }) => {
    ...
    // AUDIO NOTIFICATION
    useEffect(() => {
        // Explicitly cast to User[] to avoid 'unknown' type errors when accessing unreadCount
        const totalUnread = (Object.values(users) as User[]).reduce((acc: number, u: User) => acc + (u.unreadCount || 0), 0);
